...
## 公開 LOD (千葉市6区、127,883 トリプル)

```text
https://raw.githubusercontent.com/ta2tree101343-afk/Medical-Access-LOD/main/lod/medical-access-lod.ttl     (8.1 MB, Turtle)
https://raw.githubusercontent.com/ta2tree101343-afk/Medical-Access-LOD/main/lod/medical-access-lod.jsonld  (16.9 MB, JSON-LD)
...
- **具象クラス** `schema:Hospital` / `schema:MedicalClinic` / `schema:Dentist`（SHACL `sh:targetClass` は具象を指定）
...
その他 (施設数 / 結果行数): 内科 265/265・土曜小児科 81/103・診療科カタログ 85 / ラベル検索 / 区別集計 / bbox (216 施設) — 詳細は [`queries-real/`](queries-real/)。
...
- pytest 145 件（unit / integration / contract）+ Jest 34 件（CDK）
...
詳細は [`LICENSE`](LICENSE) 参照。

- **コード** (`src/`, `infra/`, `tests/`, `ontology/`, `queries*/`): MIT License
- **LOD データ** (`lod/*.ttl`, `lod/*.jsonld`, `lod/statistics.json`): 公共データ利用規約 (PDL) 1.0 に基づき再配布。出典「厚生労働省 医療情報ネット」明記のうえ利用可
